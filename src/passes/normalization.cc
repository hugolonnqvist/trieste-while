    ...
    PassDef normalization() {
		PassDef normalization = {
            ...
            dir::topdown,
			{
                In(Program) * T(Stmt)[Stmt] >>
                    [](Match &_) -> Node
                    {
						...
						if ((stmt / Stmt) != Semi) {
							...
							return Instructions << (Normalize << (Stmt << (Semi << stmt)));
						}
                        return Instructions << (Normalize << stmt);
                    },

                T(Normalize) << (T(Stmt) << T(Semi, Assign, If, While, Output, Skip)[Stmt]) >>
                    [](Match &_) -> Node
                    {
                        return Stmt << (Normalize << _(Stmt));
                    },

                T(Normalize) << T(Semi)[Semi] >>
                    [](Match &_) -> Node
                    {
						Nodes res = Nodes();
                ...
                T(Normalize) << (T(Assign) << (T(Ident)[Ident] * (T(AExpr) << T(Add, Sub, Mul)[Op]))) >>
                    [](Match &_) -> Node
                    {
                        ...
                        return Assign << _(Ident) 
									  << (AExpr << (op->type() << (Normalize << lhs) 
															   << (Normalize << rhs)));
                    },

                T(Normalize) << (T(Assign) << (T(Ident)[Ident] * 
				...
				T(Normalize) << (T(Output) << T(AExpr)[AExpr]) >>
					[](Match &_) -> Node
					{
						return Output << (Normalize << _(AExpr));
					},
				...
				T(Normalize) << (T(BExpr) << T(LT, Equals)[Op]) >>
                    ...
                    {	
						...
						Node lhs = _(Op) / Lhs;
						Node rhs = _(Op) / Rhs;
						return BExpr << (op_type << (Normalize << lhs)
												 << (Normalize << rhs));
                    },
